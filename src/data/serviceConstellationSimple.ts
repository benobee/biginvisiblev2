// Simplified service constellation data without Babylon.js dependencies
...
export interface SimpleTouchPoint {
  id: string;
  name: string;
  description: string;
  color: SimpleColor;
...
export interface SimpleServiceConstellation {
  id: string;
  name: string;
  description: string;
  color: SimpleColor;
...
const colors = {
  brandStrategy: { r: 0.25, g: 0.5, b: 1 },      // #4080ff
  visualIdentity: { r: 1, g: 0.25, b: 0.5 },     // #ff4080
  digitalExperience: { r: 0.25, g: 1, b: 0.5 },  // #40ff80
  contentStrategy: { r: 1, g: 0.5, b: 0.25 },    // #ff8040
  brandArchitecture: { r: 0.5, g: 0.25, b: 1 },  // #8040ff
  communityBuilding: { r: 1, g: 1, b: 0.25 },    // #ffff40
};

export const simpleServiceConstellations: SimpleServiceConstellation[] = [
  {
    id: 'brand-strategy',
    name: 'Brand Strategy',
...
export const getAllSimpleServiceTouchPoints = (): SimpleTouchPoint[] => {
  return simpleServiceConstellations.flatMap(constellation => 
    constellation.touchPoints.filter(tp => tp.type === 'service')
  );
};

// Helper function to get all feature touch points for a service
export const getSimpleFeatureTouchPoints = (serviceId: string): SimpleTouchPoint[] => {
  const constellation = simpleServiceConstellations.find(c => c.id === serviceId);
